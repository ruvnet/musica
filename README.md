...
| **Settings broadcast** | Signed-in operators can broadcast their live settings and appear in a directory ranked by concurrent listeners (top 5). Following adopts the broadcaster's tempo, Lyria direction (config plus resolved prompts), deck mix, look, and master FX, and keeps tracking them. Settings move, not audio — followers re-synthesize locally, so it shares a set's direction rather than its waveform (ADR-182). Incoming state is clamped, slewed, and never auto-starts audio. Use RESTREAM (ADR-174) for a real stream. |
