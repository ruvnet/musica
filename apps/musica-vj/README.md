...
The July 2026 live direction is Lyria RealTime, not batch MP3 slicing. The right rail runs three named Lyria WebSocket decks concurrently: `main` produces the style-directed performance bed, `sequence` receives the editable Musica step/pulse state as compact 16-step hit/rest strings plus note and lane-volume data, and `vocal` uses Lyria's `VOCALIZATION` mode for wordless voice textures. Sequence lane mute/solo state also drives Lyria's drum mute, bass mute, density, brightness, guidance, temperature, and top-K controls. Each stream has live mute, volume, semitone pitch, beat-nudge, activity, and PCM counters; the sequence controls are mirrored directly above the step grid for performance use. Pitch changes compensate the source BPM before resampling so the audible result remains locked to the master tempo; all decks share the current key/scale and transport BPM. When realtime is active, the local synth guide defaults to zero so the audible sequence layer is the Lyria stream; local synthesis remains the fallback when realtime is unavailable.
